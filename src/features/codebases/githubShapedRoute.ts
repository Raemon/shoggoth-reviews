...
import { branchPrefix } from '@/features/pull-requests/branches';
...
async function resolvedRefRoute({ owner, repo, segments }: Extract<GithubTarget, { kind: 'ref' }>): Promise<string> {
  const branch = await branchPrefix(owner, repo, segments);
