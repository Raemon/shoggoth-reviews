...
export const DEFAULT_SETTINGS: Settings = { theme: 'system', blankLines: 'half' };

export function parseSettings(stored: unknown): SavedSettings {
  const record = asRecord(stored);
  return { ...choice(record, 'theme', THEME_SETTINGS), ...choice(record, 'blankLines', BLANK_LINE_SETTINGS) };
}
